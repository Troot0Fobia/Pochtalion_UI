...
function filterEntities() {
    ...
    bridge.changeSettings(
        JSON.stringify({ key: "dialog_filters", value: values }),
    );
}
...
function filterData(elem) {
    ...
    document.querySelectorAll(".dialog").forEach((dialog) => {
        ...
        const name =
            dialog.querySelector(".name")?.textContent?.toLowerCase() || "";
...
function renderDialogs(dialogs_raw) {
    ...
    const session_filename =
        select_elem.options[select_elem.selectedIndex].textContent;
    dialogs.forEach((dialog) => {
        ...
        dialog_div.onclick = () => {
            document
                .querySelectorAll(".dialog.active")
...
function renderFilters(settings_str) {
    ...
    document
        .querySelectorAll('.filter-menu input[type="checkbox"]')
        .forEach((checkbox, index) => (checkbox.checked = settings[index]));
}
...
function changeSession() {
    ...
    const session_file =
        select_session.options[select_session.selectedIndex].textContent;
    bridge.changeSession(
        JSON.stringify({ session_id: session_id, session_file: session_file }),
    );
}
