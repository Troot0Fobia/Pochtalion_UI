...
from telethon.errors import (ForbiddenError, InputUserDeactivatedError, PeerFloodError)
...
class ChatBridge(BaseBridge):
    ...
    async def sendMessage(self, message):
        ...
        error_message = (
            f"Ошибка отправки сообщения для сессии {session_file} по причине: "
        )
        try:
            ...
            await session_manager.sendMessage(
                session_file, self.main_window.current_chat, message
            )
            return
        except PeerFloodError as e:
            self.logger.error(
                f"{self.__class__.__name__}\tCaught Flood Error, session: {session_file}: {e}",
                exc_info=True,
            )
            error_message += "Получен флуд"
        except InputUserDeactivatedError as e:
            self.logger.error(
                f"{self.__class__.__name__}\tCaught User Deactivated Error, session: {session_file}: {e}",
                exc_info=True,
            )
            error_message += "Пользователь удален"
        except ForbiddenError as e:
            self.logger.error(
                f"{self.__class__.__name__}\tCaught Forbidden Error, session: {session_file}: {e}",
                exc_info=True,
            )
            error_message += "Пользователь ограничил доступ"
        except Exception as e:
            self.logger.error(
                f"{self.__class__.__name__}\tUnexpected error while sending message, session: {session_file}: {e}",
                exc_info=True,
            )
    ...
    async def openVideoPlayer(self, video_path):
        ...
        try:
            ...
            self.logger.error(
                f"{self.__class__.__name__}\tError while opening video player: {e}",
                exc_info=True,
            )
