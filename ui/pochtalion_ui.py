...
class Pochtalion_UI(QMainWindow):
    ...
    def initSessionManager(self):
        ...
        try:
            ...
            if not re.fullmatch(r"\d{5,8}", api_id) or not re.fullmatch(
                r"[a-fA-F0-9]{32}", api_hash
            ):
                raise ValueError
            self._session_manager = SessionsManager(
                int(api_id), api_hash, self.database, self
            )
    ...
    def session_manager(self):
        if self._session_manager is None:
            self.show_notification(
                "Внимание", "Неверные api ключи. Пожалуйста введите корректные данные"
            )
            return None
        else:
        ...
        for item in TMP.iterdir():
            if item.is_dir():
                ...
                def on_rm_error(func, path, exc_info):
                    self.logger.error(
                        f"Error while deleting folder {path}: {exc_info[1]}"
                    )
